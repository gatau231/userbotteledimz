    ...
    for dialog in groups:
        ...
        try:
            if reply_message.media:
                ...
                await client.send_file(dialog.id, media_path, caption=append_watermark_to_message(reply_message.message, parse_mode='markdown'))
            else:
                message_with_watermark = append_watermark_to_message(reply_message.message)
                await client.send_message(dialog.id, message_with_watermark, parse_mode='markdown')
            sent_count += 1
